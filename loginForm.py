...
def on_closing(df):
    ...
    if len(signed_in) > 0:
        if messagebox.askokcancel("Quit", f"Do you want to quit? The following people have not signed out and will be set to a default of 1 hour: {list_to_string(signed_in)}"):
...
root.bind("<Down>", lambda event: name_box.curselection()[0] + 1 if name_box.curselection()[0] < len(names) - 1 else len(names) - 1)
